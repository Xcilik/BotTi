...
async function GroupUpdate(farid, m, store) {
	if (!m.messageStubType || !m.isGroup) return
	if (global.db?.groups[m.chat]?.setinfo && farid.public) {
		...
		if (messages[m.messageStubType]) {
			await farid.sendMessage(m.chat, { text: `${admin} ${messages[m.messageStubType]}`, mentions: [m.sender, ...(m.messageStubParameters[0]?.includes('@') ? [`${m.messageStubParameters[0]}`] : [])]}, { ephemeralExpiration: m.expiration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		} else {
			console.log({
...
async function GroupCacheUpdate(farid, update, store, groupCache) {
	try {
		for (let n of update) {
...
async function GroupParticipantsUpdate(farid, { id, participants, author, action }, store, groupCache) {
	try {
		...
		if (global.db?.groups[id] && store?.groupMetadata[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await farid.profilePictureUrl(n, 'image');
				} catch {
				...
				if (messageText && farid.public) {
					await farid.sendMessage(id, {
...
async function LoadDataBase(farid, m) {
	try {
...
async function MessagesUpsert(farid, message, store, groupCache) {
	try {
		let botNumber = await farid.decodeJid(farid.user.id);
		const msg = message.messages[0];
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) {
			store.groupMetadata ??= await farid.groupFetchAllParticipating().catch(e => ({}))
		}
		...
		const m = await Serialize(farid, msg, store, groupCache)
		require('../farid')(farid, m, msg, store, groupCache);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			await farid.appendResponseMessage(m, JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id);
		}
		if (global.db?.set[botNumber] && global.db?.set[botNumber]?.readsw) {
			if (msg.key.remoteJid === 'status@broadcast') {
				await farid.readMessages([msg.key]);
				if (/protocolMessage/i.test(type)) farid.sendFromOwner(global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					await farid.sendFromOwner(global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
				}
			}
		}
	} catch (e) {
...
async function Solving(farid, store) {
	farid.serializeM = (m) => MessagesUpsert(farid, m, store)
	...
	farid.getName = (jid, withoutContact  = false) => {
		const id = farid.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || farid.groupMetadata(id) || {};
	...
	farid.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await farid.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await farid.getName(i + '@s.whatsapp.net')}\nFN:${await farid.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await farid.getName(i + '@s.whatsapp.net')}\nFN:${await farid.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/farid_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		farid.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
	}
	
	farid.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await farid.query({
			tag: 'iq',
			attrs: {
	...
	farid.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return farid.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted.expiration || 0 })
	}
	
	farid.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return farid.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime && mime === 'application/pdf') {
				return farid.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('image')) {
				return farid.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('video')) {
				return farid.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return farid.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('audio')) {
				return farid.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			}
		}
	...
	farid.sendGroupInvite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
		...
		const invite = await farid.relayMessage(participant, message.message, { messageId: message.key.id })
		return invite
	}
	
	farid.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await farid.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted });
		}
	}
	
	farid.sendTextMentions = async (jid, text, quoted, options = {}) => farid.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	farid.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return farid.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
	}
	
	farid.downloadMediaMessage = async (message) => {
	...
	farid.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		const buffer = await farid.downloadMediaMessage(message);
	...
	farid.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: farid.user.id, quoted: m.quoted });
		apb.key = m.key
		apb.key.fromMe = areJidsSameUser(m.sender, farid.user.id);
		if (m.isGroup) apb.participant = m.sender;
		farid.ev.emit('messages.upsert', {
			...m,
	...
	farid.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await farid.getFile(path, true);
		...
		let anu = await farid.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
		await fs.unlinkSync(pathFile);
		return anu;
	}
	
	farid.sendListMsg = async (jid, content = {}, options = {}) => {
		...
		const hasil = await farid.relayMessage(msg.key.remoteJid, msg.message, {
	...
	farid.sendButtonMsg = async (jid, content = {}, options = {}) => {
	...
	farid.newsletterMsg = async (key, content = {}, timeout = 5000) => {
		...
		if (react) {
			...
			const msg = await generateWAMessageContent(media, { upload: farid.waUploadToServer });
			const anu = await farid.query({
				tag: 'message',
			...
			const _query = await farid.query({
				tag: 'iq',
				attrs: {
					to: 's.whatsapp.net',
					type: 'get',
	...
	farid.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: farid.waUploadToServer });
			return imageMessage;
		}
		...
		const hasil = await farid.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	if (farid.user && farid.user.id) {
		const botNumber = farid.decodeJid(farid.user.id);
		if (global.db?.set[botNumber]) {
			farid.public = global.db.set[botNumber].public
		} else farid.public = true
	} else farid.public = true
...
async function Serialize(farid, m, store, groupCache) {
	const botNumber = farid.decodeJid(farid.user.id)
	if (!m) return m
	...
	if (m.key) {
		...
		m.sender = farid.decodeJid(m.fromMe && farid.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await farid.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = groupCache.get(m.chat))
			if (!metadata) {
				metadata = await farid.groupMetadata(m.chat).catch(e => ({}))
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = farid.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === farid.decodeJid(farid.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, farid)
				return await Serialize(farid, q, store, groupCache)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(farid.decodeJid(m.msg?.contextInfo?.participant), farid.decodeJid(farid?.user?.id)),
			...
			m.quoted.download = () => farid.downloadMediaMessage(m.quoted)
			m.quoted.delete = () => {
				farid.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.download = () => farid.downloadMediaMessage(m)
	
	m.copy = () => Serialize(farid, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
	...
	m.reply = async (content, options = {}) => {
		...
		if (typeof content === 'object') {
			return farid.sendMessage(chat, content, { ...options, quoted, ephemeralExpiration })
			...
			try {
				if (/^https?:\/\//.test(content)) {
					...
					if (/gif|image|video|audio|pdf|stream/i.test(mime)) {
						return farid.sendMedia(chat, data.data, '', caption, quoted, content)
					} else {
						return farid.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
					}
				} else {
					return farid.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
				}
